...
public class MapAnd2DArrayHomework {
	...
	public static HashMap<String, ArrayList<Integer>> reverseHashmap(HashMap<Integer, String> initialMap) {
		...
		for (Integer number : initialMap.keySet()) {
			character = initialMap.get(number);
			if (output.containsKey(character)) {
				output.get(character).add(number);
			} else {
				ArrayList<Integer> values = new ArrayList<>();
				values.add(number);
				output.put(initialMap.get(number), values);
			}
		}
		return output;
	}
	...
	public static int getNumberOfCoursesToTake(HashMap<String, String> courseMap, String course) {
		...
		while (!courseMap.get(course).equals("")) {
			...
			for (char character : row) {
				if (score.containsKey(character)) {
					if (previous == character) {
						...
						if (score.get(character) > leader_score) {
							leader_score = score.get(character);
						}
					}
				} else {
					score.put(character, 1);
				}
				previous = character;
			}
		}
