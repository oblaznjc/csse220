...
/**
 * This class stores information about a city and can generate a report based on
 * that information. It should be able to handle a number of different bad data
 * examples as specified in the README.md file
 * 
 * @author oblaznjc
 *
 */
public class City {
	...
	public void addData(String s) throws InvalidDataException {

		String[] data = s.split(",");

		// TODO this code cannot currently handle a line that doesn't have 3 integers
		// separated by commas
		...
		try {
			...
			if (monthToInfectionMap.containsKey(month)) {
				if (monthToInfectionMap.get(month) == infected && monthToDeathMap.get(month) == deaths) {
					...
					throw new DataDuplicationException("Duplicate month data with differing values found!");

					// TODO should throw an Exception so we can report this with the filename and
					// line number
			...
			throw new InvalidDataException("did NOT include all 3 parameters!");
		} catch (NumberFormatException e) {
			throw new InvalidDataException("did NOT include all month, infection, and deaths as integers!");
		}
	}
