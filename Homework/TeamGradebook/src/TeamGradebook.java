...
public class TeamGradebook {
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		// TODO: Your code here
		ArrayList<Student> currentTeam = new ArrayList<>();
		for (String name : memberNames) { // create array of students using the given names
			if (!students.contains(name)) {
				this.handleAddStudent(name);
			}
			Student student = getStudentByName(name);
			this.students.add(student);
			currentTeam.add(student);
		}
		Team team = new Team(teamName, currentTeam);
		teams.add(team);
		return "ok";
	}
	...
	private String handleGetBestTeam() {
		...
		for(Team team : this.teams) {
			if(team.getAverage() > bestScore) {
				bestTeam = team.getName();
				bestScore = team.getAverage();
			}
		}
		return bestTeam;
	}
	...
	/**
	 * Returns the team from teams with the particular name, null otherwise
	 * (e.g., if the team does not exist).
	 * 	 * 
	 * @param name the name of the team for whom to search
	 * @return team object with the name or null
	 */
