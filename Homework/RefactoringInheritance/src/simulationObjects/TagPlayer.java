...
/**
 * The TagPlayer class represents a person playing the classic game of "tag."
 * https://en.wikipedia.org/wiki/Tag_(game)
 * 
 * A Person can either be "it" or not. When a person is "it" then they appear
 * with the color RED. A person moves around within an area and bounces off of
 * the boundaries and also off of other TagPlayers.
 * 
 * When a TagPlayer bounces into another then if one person is "it" and the
 * other is not, then they switch roles. The person who was not "it" becomes
 * "it" and vice versa.
 * 
 * One more rule is that a player who most recently "it" cannot become it again
 * for 10 timesteps. This is to prevent "tagbacks" where a player immediately
 * tags back the person who just tagged them.
 * 
 ...
 *
 */
public class TagPlayer extends SimulationObject {
	...
	public TagPlayer(int x, int y, boolean isIt) {
		...
		setRadius(TAG_PLAYER_RADIUS);
		setMoveSpeed(TAG_PLAYER_SPEED);
	...
	/**
	 * Move in opposite direction of collision with other TagPlayer. Also do checks
	 * to see if someone got tagged
	 */
	 ...
	 * THIS CODE IS UNIQUE TO TagPlayer
	 * 
	 * It checks to see if the other player is "it" to determine if the current
	 * player (this) should become "it" and then make the change accordingly,
	 * including making the other player NOT "it" and also briefly immune
	 * (COUNTDOWN_RESET) from being tagged back.
	 * 
	 * @param other
	 */
