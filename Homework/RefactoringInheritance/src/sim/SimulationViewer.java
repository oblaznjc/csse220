 ...
 * 
 * This class is where the program begins and it provides the setup and
 * configuration of various GUI pieces.
 * 
 * @author Jason Yoder
 *
 */
public class SimulationViewer {
	...
	public SimulationViewer() {
		...
		// Modify this if you wish to add different numbers of things into the
		// simulation
		Integer[] numbersForDropdown = { 1, 10, 100 };
		// create a combo box with the fixed array so you can pick how many things to
		// add
		...
		// TODO Part 3 add buttons to add ChargedParticles, see JButton addParticle for
		// an example

		// Button for adding Charged Particles to a simulation
		JButton addChargedParticles = new JButton("Add Charged Particles");
		addChargedParticles.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// passes in the number from the dropdown list
		...
		buttonPanel.add(addChargedParticles);

		// TODO Part 4 add buttons to add Persons, see JButton addParticle for an
		// example
		JButton addPersons = new JButton("Add Persons");
		addPersons.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// passes in the number from the dropdown list
				simulationComponent.addPersons((Integer) addNumberSelector.getSelectedItem());
			}
		});
		buttonPanel.add(addPersons);
