 ...
 * 
 * It is also connected to a PandemicStatsComponent to report the changing rate
 * of health status for a population of people.
 * 
 * @author Jason Yoder
 *
 */
public class SimulationComponent extends JComponent {
	...
	// TODO Part 3 add method for adding ChargedParticles, see addParticles() for an
	// example

	public void addChargedParticles(int count) {
		for (int i = 0; i < count; i++) {
			this.particles.add(new ChargedParticle(rand.nextInt(this.getWidth()), rand.nextInt(this.getHeight())));
		}
	}
	 ...
	 * 
	 * Make sure that you add one infected person in order to see how infect
	 * spreads.
	 * 
	 * @param count
	 */
	public void addPersons(int count) {
		...
		for (int i = 0; i < count; i++) {
			this.persons.add(new Person(rand.nextInt(this.getWidth()), rand.nextInt(this.getHeight()), false));
		}
		// Add at least one Person who is "it"
		Person infectedPerson = new Person(rand.nextInt(this.getWidth()), rand.nextInt(this.getHeight()), true);
		infectedPerson.setInfectedStatus(true);
		this.persons.add(infectedPerson);
	}
	 ...
	 * You do NOT need to change this method
	 * 
	 * This method simply loops through every kind of simulation object and calls
	 * its update() method so that it will move appropriately. It also checks to see
	 * if specific types of objects bump into each other and if so calls the
	 * appropriate code.
	 */
	public void update() {
	...
	/**
	 * This code should not need to be changed. You need to implement code in Person
	...
	private void logPandemicInformation() {
		...
		if (healthy + sick + recovered + dead > 0) {
			this.statsComponent.addEntry(healthy, sick, recovered, dead);
		}
	}
}
