...
public class TestThisClass {
	...
	/**
	 * ensures: returns an exact count of the number of 'X' or 'x' characters
	 * appearing in inputString
	 * 
	...
	public static int numberOfXs(String inputString) {
		...
		for (int i = 0; i < inputString.length(); i++) {
			//System.out.println(inputString.charAt(i) + " / 8 , returns:" + inputString.charAt(i) / 8);
			//System.out.println(inputString.charAt(i) / 8);
	...
	/**
	 * ensures: returns an exact count of the number of times "Chocula" appears in
	 * inputString
	 * 
	 * @param inputString is analyzed for "Chocula" - case sensitive, e.g.,
	 *                    "chocula" will not be counted
	...
	/**
	 * ensures: returns -1 if any input parameter is not positive, otherwise <br>
	 * returns the cost in dollars to paint a room's floor based on the input
	 * parameters
	 * 
	 ...
	 * @return cost to paint a room's floor or -1 if any input parameter is not
	 *         positive
	 */
	...
	/**
	 * ensures: returns the cost in dollars for a pizza made with the toppings found
	 * in toppingsList <br>
	 * cost = $8 for zero toppings <br>
	 * For greater than 0 toppings in toppingsList: <br>
	 * cost = $8 + $2 each for toppings #1 and #2 and then add $1 each for toppings
	 * #3, #4, etc. <br>
	 * cost = $12 for a 'special' <br>
	 * a 'special' only has these 3 toppings (in any order): "pepperoni", "onions"
	 * and "sausage"
	 * 
